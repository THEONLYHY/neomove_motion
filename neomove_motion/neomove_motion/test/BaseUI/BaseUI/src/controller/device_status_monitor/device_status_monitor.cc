...
void DeviceStatusMonitor::Init() {
  ...
  yotta::IoConfigPtr io_config = yotta::ConfigFactory::GetInstance()->GetIoConfig();
  yotta::AxisConfigPtr axis_config = yotta::ConfigFactory::GetInstance()->GetAxisConfig();

  // Load Analog IOs
  if (io_config) {
    yotta::MultiAnalogIoPortConfigPtr analog_io_config = io_config->GetAnalogIo();
    if (analog_io_config) {
      for (int i = 0; i < analog_io_config->GetAnalogIoPortConfigCount(); ++i) {
        yotta::AnalogIoPortConfigPtr port = analog_io_config->GetAnalogIoPortConfig(i);
        char id_buf[256] = {0};
  ...
  // Load Input IOs
  if (io_config) {
    yotta::MultiIoPortConfigPtr input_config = io_config->GetInputIo();
    if (input_config) {
      for (int i = 0; i < input_config->GetIoPortConfigCount(); ++i) {
        yotta::IoPortConfigPtr port = input_config->GetIoPortConfig(i);
        char id_buf[256] = {0};
        port->GetIds(id_buf, 256);
        std::string id = id_buf;
    ...
    int axis_io_count = io_config->GetAxisIoCount();
    for (int i = 0; i < axis_io_count; ++i) {
      yotta::AxisIoConfigPtr axis_io_cfg = io_config->GetAxisIo(i);
      for (int j = 0; j < axis_io_cfg->GetIoPortConfigCount(); ++j) {
        yotta::IoPortConfigPtr port = axis_io_cfg->GetIoPortConfig(j);
        char id_buf[256] = {0};
        port->GetIds(id_buf, 256);
        std::string id = id_buf;
  ...
  // Load Output IOs
  if (io_config) {
    yotta::MultiIoPortConfigPtr output_config = io_config->GetOutputIo();
    if (output_config) {
      for (int i = 0; i < output_config->GetIoPortConfigCount(); ++i) {
        yotta::IoPortConfigPtr port = output_config->GetIoPortConfig(i);
        char id_buf[256] = {0};
        port->GetIds(id_buf, 256);
        std::string id = id_buf;
  ...
  if (axis_config) {
    for (int i = 0; i < axis_config->GetAxisCount(); ++i) {
      yotta::AxisConfigItemPtr axis_item = axis_config->GetAxis(i);
      char id_buf[256] = {0};
...
void DeviceStatusMonitor::MonitoringLoop() {
  ...
  while (is_running_) {
    ...
    // Poll Analog
    for (const AnalogIoInfo& item : analog_ios_) {
      short val = 0;
    ...
    // Poll Inputs
    for (const InputIoInfo& item : input_ios_) {
      unsigned char val = 0;
    ...
    // Poll Outputs
    for (const OutputIoInfo& item : output_ios_) {
      unsigned char val = 0;
      if (item.ptr->ReadValue(&val) == 0) {
    ...
    {
      std::unique_lock<std::shared_mutex> lock(mutex_);
      for (const std::pair<std::string, double>& val : temp_analog_values)
        analog_io_cache_[val.first] = val.second;
      for (const std::pair<std::string, uint8_t>& val : temp_input_values)
        input_io_cache_[val.first] = val.second;
      for (const std::pair<std::string, uint8_t>& val : temp_output_values)
        output_io_cache_[val.first] = val.second;
      for (const std::pair<std::string, int>& val : temp_axis_states)
        axis_state_cache_[val.first] = val.second;
      for (const std::pair<std::string, int>& val : temp_axis_home_states)
        axis_home_state_cache_[val.first] = val.second;
      for (const std::pair<std::string, double>& val : temp_axis_positions)
...
bool DeviceStatusMonitor::GetInputIoValue(const std::string& id,
  ...
  std::map<std::string, uint8_t>::iterator it = input_io_cache_.find(id);
...
bool DeviceStatusMonitor::GetAnalogIoValue(const std::string& id,
                                           double& value) {
  std::shared_lock<std::shared_mutex> lock(mutex_);
  std::map<std::string, double>::iterator it = analog_io_cache_.find(id);
...
bool DeviceStatusMonitor::GetOutputIoValue(const std::string& id,
                                           uint8_t& value) {
  std::shared_lock<std::shared_mutex> lock(mutex_);
  std::map<std::string, uint8_t>::iterator it = output_io_cache_.find(id);
...
bool DeviceStatusMonitor::GetAxisState(const std::string& id, int& state) {
  std::shared_lock<std::shared_mutex> lock(mutex_);
  std::map<std::string, int>::iterator it = axis_state_cache_.find(id);
...
bool DeviceStatusMonitor::GetAxisHomeState(const std::string& id, int& state) {
  std::shared_lock<std::shared_mutex> lock(mutex_);
  std::map<std::string, int>::iterator it = axis_home_state_cache_.find(id);
...
bool DeviceStatusMonitor::GetAxisPos(const std::string& id, double& pos) {
  std::shared_lock<std::shared_mutex> lock(mutex_);
  std::map<std::string, double>::iterator it = axis_pos_cache_.find(id);
