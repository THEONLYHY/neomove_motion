...
LimitSettingView::LimitSettingView(QWidget* parent)
    : QDialog(parent), ui(new Ui::limit_condition_viewClass()) {
  ui->setupUi(this);
  ...
  connect(ui->button_add_axis_states_, &QPushButton::clicked, this,
                   &LimitSettingView::OnAddLimitAxisStatesButtonClicked);
  connect(ui->button_del_axis_states_, &QPushButton::clicked, this,
                   &LimitSettingView::OnDelLimitAxisStatesButtonClicked);
  connect(ui->button_add_axis_region_, &QPushButton::clicked, this,
                   &LimitSettingView::OnAddLimitAxisRegionStatesButtonClicked);
  connect(ui->button_del_axis_region_, &QPushButton::clicked, this,
                   &LimitSettingView::OnDelLimitAxisRegionStatesButtonClicked);
  connect(ui->button_add_io_states_, &QPushButton::clicked, this,
                   &LimitSettingView::OnAddLimitIOStatesButtonClicked);
  connect(ui->button_del_io_states_, &QPushButton::clicked, this,
                   &LimitSettingView::OnDelLimitIOStatesButtonClicked);
  connect(ui->button_add_limit_, &QPushButton::clicked, this,
                   &LimitSettingView::OnAddLimitButtonClicked);
  connect(ui->button_del_limit_, &QPushButton::clicked, this,
                   &LimitSettingView::OnDelLimitButtonClicked);
  connect(ui->button_save_limit_, &QPushButton::clicked, this,
...
PsComboBox* LimitSettingView::GetAxisLimitTypeComboBox() {
  PsComboBox* axis_limit_type_combox = new PsComboBox;
...
PsComboBox* LimitSettingView::GetIoLimitTypeComboBox() {
  PsComboBox* io_limit_type_combox = new PsComboBox;
...
PsComboBox* LimitSettingView::GetAxisNameComboBox() {
  PsComboBox* axis_name_combox = new PsComboBox;
...
PsComboBox* LimitSettingView::GetAxisStatesComboBox() {
  PsComboBox* axis_move_states_combox = new PsComboBox;
  axis_move_states_combox->addItem(tr("STOP"));
  axis_move_states_combox->addItem(tr("MOVING"));
  return axis_move_states_combox;
}
PsComboBox* LimitSettingView::GetAxisRegionNameComboBox(std::string axis_ids) {
  PsComboBox* axis_region_name_combox = new PsComboBox;
...
PsComboBox* LimitSettingView::GetAxisRegionStatesComboBox() {
  PsComboBox* axis_region_states_combox = new PsComboBox;
...
PsComboBox* LimitSettingView::GetIONameComboBox() {
  PsComboBox* io_name_combox = new PsComboBox;
...
PsComboBox* LimitSettingView::GetIOStatesComboBox() {
  PsComboBox* axis_io_states_combox = new PsComboBox;
...
void LimitSettingView::OnLimitListSelect(int row, int column) {
  ...
  if (limit_condition_config) {
    ...
    for (int j = 0; j < limit_condition_config->GetAxisStateConditionCount();
      ...
      PsComboBox* axis_name_combox = GetAxisNameComboBox();
      ...
      PsComboBox* axis_states_combox = GetAxisStatesComboBox();
    ...
    for (int j = 0; j < limit_condition_config->GetAxisRegionConditionCount();
      ...
      ui->tableWidget_axis_region_states_->setCellWidget(RowCont, 0,
                                                         axis_name_combox);
      // 轴名切换时，仅刷新同行的区域名可选项，不写入配置对象
      connect(
          axis_name_combox, QOverload<int>::of(&QComboBox::currentIndexChanged),
          this, [this, axis_name_combox, RowCont]() {
            QComboBox* region_combox = qobject_cast<QComboBox*>(
                ui->tableWidget_axis_region_states_->cellWidget(RowCont, 1));
            if (!region_combox) return;
            region_combox->clear();
            std::string axis_ids =
                axis_name_combox->currentText().toStdString();
            yotta::AxisConfigItemPtr axis_cofig_item =
                axis_config_->GetAxisByIds(axis_ids.c_str());
            if (!axis_cofig_item) return;
            for (int i = 0; i < axis_cofig_item->GetRegionCount(); i++) {
              yotta::AxisRegionConfigPtr axis_region =
                  axis_cofig_item->GetRegion(i);
              if (!axis_region) continue;
              std::string region_ids;
              char region_ids_char[256];
              axis_region->GetIds(region_ids_char, 256);
              region_ids = region_ids_char;
              region_combox->addItem(QString::fromStdString(region_ids));
            }
          });

      PsComboBox* axis_region_name_combox = GetAxisRegionNameComboBox(axis_ids);
      ...
      PsComboBox* axis_region_states_combox = GetAxisRegionStatesComboBox();
    ...
    for (int j = 0; j < limit_condition_config->GetIOConditionCount(); j++) {
      ...
      PsComboBox* io_name_combox = GetIONameComboBox();
      ...
      PsComboBox* io_states_combox = GetIOStatesComboBox();
  ...
  if (limit_type_) {
    ...
    if (io_limit_rule_config) {
      for (int i = 0; i < io_limit_rule_config->GetLimitConditionCount(); i++) {
        ...
        PsComboBox* mode_combox = GetIoLimitTypeComboBox();
        switch (type) {
    ...
    if (axis_limit_rule_config) {
      for (int i = 0; i < axis_limit_rule_config->GetLimitConditionCount();
        ...
        ui->limit_list->setItem(
            RowCont, 0,
            new QTableWidgetItem(QString::fromStdString(condition_ids)));
        PsComboBox* mode_combox = GetAxisLimitTypeComboBox();
        switch (type) {
...
void LimitSettingView::ReadLimitCondition() {
  ...
  if (limit_type_) {
    ...
    limit_condition_config =
        io_limit_rule_config->GetLimitCondition(limit_index_);
    if (limit_condition_config) {
      QTableWidgetItem* io_item = ui->limit_list->item(limit_index_, 0);
      if (!io_item) return;
      std::string rule_ids = io_item->text().toStdString();
      limit_condition_config->SetRuleIds(rule_ids.c_str());

      PsComboBox* io_limit_combox = qobject_cast<PsComboBox*>(
          ui->limit_list->cellWidget(limit_index_, 1));
    ...
    limit_condition_config =
        axis_limit_rule_config->GetLimitCondition(limit_index_);
    if (limit_condition_config) {
      QTableWidgetItem* axis_item = ui->limit_list->item(limit_index_, 0);
      if (!axis_item) return;
      std::string rule_ids = axis_item->text().toStdString();
      limit_condition_config->SetRuleIds(rule_ids.c_str());
      PsComboBox* axis_limit_combox = qobject_cast<PsComboBox*>(
          ui->limit_list->cellWidget(limit_index_, 1));
  ...
  if (limit_condition_config) {
    ...
    for (int i = 0; i < row_count_axis_states; i++) {
      PsComboBox* axis_name_combox = qobject_cast<PsComboBox*>(
          ui->tableWidget_axis_states_->cellWidget(i, 0));
      PsComboBox* axis_states_combox = qobject_cast<PsComboBox*>(
    ...
    for (int i = 0; i < row_count_axis_region; i++) {
      ...
      PsComboBox* axis_region_name_combox = qobject_cast<PsComboBox*>(
          ui->tableWidget_axis_region_states_->cellWidget(i, 1));
      PsComboBox* axis_region_states_combox = qobject_cast<PsComboBox*>(
    ...
    for (int i = 0; i < row_count_io_states; i++) {
      PsComboBox* io_name_combox = qobject_cast<PsComboBox*>(
          ui->tableWidget_io_states_->cellWidget(i, 0));
      PsComboBox* io_states_combox = qobject_cast<PsComboBox*>(
...
void LimitSettingView::paintEvent(QPaintEvent *event) {
  if (!isWindow()) {
  ...
  ps_style::PsDrawSurface(&painter, QRectF(rect()).adjusted(0, 0, -4, -4));
}
...
void LimitSettingView::OnAddLimitAxisRegionStatesButtonClicked() {
  ...
  ui->tableWidget_axis_region_states_->setCellWidget(RowCont, 0,
                                                     axis_name_combox);
  // 轴名切换时，仅刷新同行的区域名可选项，不写入配置对象
  connect(axis_name_combox, QOverload<int>::of(&QComboBox::currentIndexChanged),
          this, [this, axis_name_combox, RowCont]() {
            QComboBox* region_combox = qobject_cast<QComboBox*>(
                ui->tableWidget_axis_region_states_->cellWidget(RowCont, 1));
            if (!region_combox) return;
